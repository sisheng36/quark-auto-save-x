...
为获得最佳使用体验，建议完成 TMDB、Trakt 和 CloudSaver/PanSou 等免费第三方服务配置。资源搜索、追剧日历、自动追更等核心功能均依赖这些服务的数据支持。
...
- 🔍 **资源搜索**：智能搜索夸克网盘资源，自动填充分享链接，支持自动过滤失效资源、屏蔽指定分享者资源
...
- 支持分享链接的子目录转存
- 支持需提取码的分享链接
- 支持定时运行和自动运行（根据节目播出时间智能执行任务，转存数达到当前季集数后自动跳过）
- 智能搜索资源并自动填充（集成 CloudSaver、PanSou，支持自动过滤失效资源）
- 记录失效分享并跳过任务
...
- 两种执行周期模式：
  - **自选模式**：按自定义周期执行
  - **自动模式**：按任务进度执行，转存数达到当前季集数后自动跳过
- 任务类型筛选（剧集、动画、综艺等）
- 任务排序（按任务编号、名称、进度、更新时间、播出时间、订阅时长）
- 任务视图切换（列表视图、海报视图）
- 支持设置默认（或继承）保存路径和命名规则

### 资源搜索

- 根据任务名称智能搜索夸克网盘资源（支持自动或手动搜索）
- 自动填充分享链接到任务配置
- 自动过滤失效资源
- 支持屏蔽指定分享者资源
- 支持设置优选分享者
- 搜索结果支持三级智能高亮（当前资源、同分享者资源、优选资源）
...
| 匹配表达式                                | 效果                                                                   |
| -------------------------------------- | ---------------------------------------------------------------------- |
| `乘风 - S07E{}`                     | 2026.04.03-第1期（上）.mp4 → 乘风 - S07E01.mp4<br>2026.04.04-第1期（下）.mkv → 乘风 - S07E02.mkv |
| `乘风 - S07E{}.mp4`                 | 2026.04.03-第1期（上）.doc → 乘风 - S07E01.mp4<br>2026.04.04-第1期（下）.docx → 乘风 - S07E02.mp4 |
...
| 匹配表达式                                | 效果                                                                   |
| -------------------------------------- | ---------------------------------------------------------------------- |
| `主角 - S01E[]`                     | 01-4K.mp4 → 主角 - S01E01.mp4<br>02-4K.mkv → 主角 - S01E02.mkv |
| `主角 - S01E[].mkv`                 | 01-4K.doc → 主角 - S01E01.mkv<br>02-4K.docx → 主角 - S01E02.mkv |
...
- 如果转存数尚未达到当前季集数，每次定时运行都会尝试转存（资源往往晚于播出才出现）
- 如果转存数已经等于当前季集数（电影已转存），后续定时任务将自动跳过该任务

转存已经全部完成的任务不会再检测分享链接是否过期。

## 项目主页

仓库内置了一份可部署到 GitHub Pages 的介绍站，源码在 [`docs/`](./docs/)。

1. 打开仓库 **Settings → Pages**
2. Build and deployment 选择 **GitHub Actions**
3. 推送到 `main` 后，工作流 [Deploy GitHub Pages](./.github/workflows/github-pages.yml) 会发布 `docs/`
